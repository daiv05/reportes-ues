<x-app-layout>
    ...
    <div class="mx-auto mt-20 px-4">
        ...
        <div class="mb-12 grid gap-8 md:m-12 md:grid-cols-2 lg:m-20 lg:gap-12">
            ...
            >
                <div class="mb-6">
                    <div class="text-lg font-bold text-escarlata-ues dark:text-orange-100 md:text-xl mb-1">
                        Reportar Nueva Incidencia
                    </div>
                    <div class="text-[14px] text-gray-800">Informa sobre un nuevo problema o solicitud</div>
                </div>
                <div>
                    <a href="{{ route('crear-reporte') }}" class="flex gap-3 text-sm justify-center items-center p-1.5 rounded-md w-full bg-escarlata-ues text-white hover:bg-orange-900/90">
            ...
            >
                ...
                <div>
                    <a href="{{ route('reportes-generales') }}" class="flex gap-3 text-sm justify-center items-center p-1.5 rounded-md w-full bg-orange-100 text-escarlata-ues hover:bg-orange-300">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-8">
            <h2 class="text-2xl font-bold text-escarlata-ues mb-4">Actividad Reciente</h2>
            <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    <h2 class="text-lg font-bold text-gray-800">Reportes Nuevos</h2>
                    <div class="text-2xl font-bold text-escarlata-ues">{{ $dataReportesNuevos['reportesNuevos7Dias'] }}</div>
                    <p class="text-xs text-gray-600">{{ $dataReportesNuevos['porcentajeReportesNuevos'] }}% desde la última semana</p>
                </div>
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    <h2 class="text-lg font-bold text-gray-800">En Proceso</h2>
                    <div class="text-2xl font-bold text-escarlata-ues">{{ $dataReportesEnProceso['reportesEnProceso7Dias'] }}</div>
                    <p class="text-xs text-gray-600">{{ $dataReportesEnProceso['porcentajeReportesEnProceso'] }}% desde la última semana</p>
                </div>
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    <h2 class="text-lg font-bold text-gray-800">Incidencias Resueltas</h2>
                    <div class="text-2xl font-bold text-escarlata-ues">{{ $dataReportesFinalizados['reportesFinalizados7Dias'] }}</div>
                    <p class="text-xs text-gray-600">{{ $dataReportesFinalizados['porcentajeReportesFinalizados'] }}% desde la última semana</p>
                </div>
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    <h2 class="text-lg font-bold text-gray-800">Incidencias Descartadas</h2>
                    <div class="text-2xl font-bold text-escarlata-ues">{{ $dataReportesNoProcede['reportesNoProcede7Dias'] }}</div>
                    <p class="text-xs text-gray-600">{{ $dataReportesNoProcede['porcentajeReportesNoProcede'] }}% desde la última semana</p>
                </div>
            </div>
            <div class="mt-6">
                <h3 class="font-bold text-lg mb-2 text-escarlata-ues">Últimas Actualizaciones</h3>
                <ul class="space-y-2">
                    @foreach ($actividadReciente as $actualizacion)
                        <li class="flex items-center space-x-2 p-2 bg-gray-50 rounded-xl hover:bg-gray-100">
                            @switch($actualizacion['acciones'])
                                @case('CREACION')
                                    <span class="w-3 h-3 bg-teal-500 rounded-full"></span>
                                    @break
                                @default
                                <span class="w-3 h-3 {{ $colors[$actualizacion['estado']] }} rounded-full"></span>
                            @endswitch
                            <span class="text-sm">
                                @switch($actualizacion['acciones'])
                                    @case('CREACION')
                                    ...
                                    @default
                                        @if ($actualizacion['estado'] === 'FINALIZADO')
                                            Se ha marcado como {{ $actualizacion['estado'] }} la incidencia #{{ $actualizacion['reporte'] }}
                                        @else
                                            @if($actualizacion['estado'] === 'ASIGNADO')
                                                Se ha asignado la incidencia #{{ $actualizacion['reporte'] }} para proceder a su resolución
                                            @else
                                                Se ha actualizado la incidencia #{{ $actualizacion['reporte'] }} a estado {{ $actualizacion['estado'] }}
                                            @endif
                                        @endif
                                @endswitch
                            </span>
        ...
        <div class="w-full mb-8 bg-white rounded-lg shadow-md overflow-hidden">
            <div class="p-8">
                <h2 class="text-2xl font-bold text-escarlata-ues mb-4">Estados de Incidencias</h2>
                <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <div class="bg-white p-4 rounded-lg shadow">
                    ...
                    <h3 class="font-semibold text-sm">NO ASIGNADO</h3>
                    </div>
                ...
                <div class="bg-white p-4 rounded-lg shadow">
                    ...
                    <h3 class="font-semibold text-sm">ASIGNADO</h3>
                    </div>
                    <p class="mt-2 text-sm text-gray-600">
                ...
                <div class="bg-white p-4 rounded-lg shadow">
                    ...
                    <h3 class="font-semibold text-sm">EN PROCESO</h3>
                    </div>
                    <p class="mt-2 text-sm text-gray-600">
                ...
                <div class="bg-white p-4 rounded-lg shadow">
                    ...
                    <h3 class="font-semibold text-sm">EN PAUSA</h3>
                    </div>
                    <p class="mt-2 text-sm text-gray-600">
                    La resolución de la incidencia está pausada debido a falta de recursos, una actividad pendiente o cualquier razón que requiera una espera.
                    </p>
                </div>
                <div class="bg-white p-4 rounded-lg shadow">
                    <div class="flex items-center gap-3">
                    ...
                    <h3 class="font-semibold text-sm">COMPLETADO</h3>
                    </div>
                    <p class="mt-2 text-sm text-gray-600">
                    La solución está parcialmente completada, ya que el trabajo ha sido realizado, pero el supervisor debe confirmarlo.
                    </p>
                </div>
                <div class="bg-white p-4 rounded-lg shadow">
                    <div class="flex items-center gap-3">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-green-700" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path>
                        <polyline points="22 4 12 14.01 9 11.01"></polyline>
                    </svg>
                    <h3 class="font-semibold text-sm">FINALIZADO</h3>
                    </div>
                    <p class="mt-2 text-sm text-gray-600">
                ...
                <div class="bg-white p-4 rounded-lg shadow">
                    ...
                    <h3 class="font-semibold text-sm">INCOMPLETO</h3>
                    </div>
                    <p class="mt-2 text-sm text-gray-600">
                ...
                <div class="bg-white p-4 rounded-lg shadow">
                    ...
                    <h3 class="font-semibold text-sm">NO PROCEDE</h3>
                    </div>
                    <p class="mt-2 text-sm text-gray-600">
                    El reporte no puede proceder porque la incidencia no se puede resolver, no es válida o es falsa.
                    </p>
                </div>
                </div>
            </div>
