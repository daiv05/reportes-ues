<x-guest-layout>
    <div class="p-4">
        <div class="mb-8 text-center">
            <h1 class="text-2xl font-bold mb-1">Verificación de correo electrónico</h1> <br />
            @if (session('code-send'))
                <p class="text-[15px] text-slate-500">Ingrese el código de 6 digitos enviado al correo que
                    proporcionó en el registro</p>
            @else
                <p class="text-[15px] text-slate-500">Antes de continuar, por favor verifique su correo electrónico</p>
            @endif
        </div>
        @if (!session('code-send'))
            <form class="mx-auto flex items-center justify-center" id="otp-form" method="POST"
                action="{{ route('verificacion-email.reenviar') }}">
                @csrf
        ...
        @else
            <form id="otp-form" method="POST" action="{{ route('verificacion-email.confirmar') }}"
