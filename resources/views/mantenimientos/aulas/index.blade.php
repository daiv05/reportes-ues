...
<x-app-layout>
    <x-slot name="header">
        ...
        <div class="p-6">
            @canany(['AULAS_CREAR'])
                ...
                <x-forms.primary-button data-modal-target="static-modal-excel" data-modal-toggle="static-modal-excel"
                    class="block" type="button">
                    Importar datos
                </x-forms.primary-button>
                <x-forms.primary-button id="descargarAulasBtn" class="block" type="button">
                    Descargar Formato Aulas
                </x-forms.primary-button>
            @endcanany
                        ...
                        <x-table.td>
                            <div class="relative flex justify-center space-x-2">
                                @canany(['AULAS_EDITAR'])
                                    ...
                                    <div id="tooltip-edit-{{ $aula->id }}" role="tooltip"
        ...
        <x-slot name="body">
            <form id="asignacion-form" method="POST" action="{{ route('aulas.store') }}">
                ...
                <x-forms.row :columns="2">
                    <div>
                        <x-forms.field id="nombre" label="Nombre" name="nombre" pattern="^[a-zA-Z0-9]{1,50}$"
                            patternMessage="Solo se permiten 50 caracteres que sean letras o números" :value="old('nombre')"
    ...
    <x-form-modal id="static-modal-excel" class="hidden">
        ...
        <x-slot name="body">
            <form id="import-excel-aulas" action="{{ route('aulas.importar') }}" method="POST"
                ...
                <div class="mx-auto flex w-full flex-col items-center justify-center gap-3">
                    <label for="file"
                        ...
                        <input type="file" name="excel_file" accept=".xls,.xlsx,.csv" id="excel_file"
        ...
        <x-slot name="footer">
            ...
            <button type="submit" form="import-excel-aulas"
                class="ms-6 rounded-lg bg-red-700 px-8 py-2.5 text-center text-sm font-medium text-white focus:outline-none focus:ring-4">
                Guardar
            </button>
        </x-slot>
...
<script>
    document.getElementById('descargarAulasBtn').addEventListener('click', function() {
        fetch('/descargar/archivo/locales', {
                method: 'GET',
                headers: {
